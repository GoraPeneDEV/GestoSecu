...
@section('title', 'Rapport de ronde par période')

@section('content')
    <a href="{{ route('sie.dashboard') }}" class="btn btn-sm btn-outline-secondary mb-3">
        <i class="ti ti-arrow-left"></i> Retour
    </a>

    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="mb-0">Rapport de ronde par période</h3>
        <a href="{{ route('sie.rapport.pdf', request()->query()) }}" class="btn btn-outline-secondary">
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form method="GET" action="{{ route('sie.rapport') }}" class="row g-2 align-items-end">
                <div class="col-auto">
    ...
    <div class="card mb-3">
        <div class="card-header">Répartition par agent</div>
        <div class="card-body p-0">
            @if ($parAgent->isEmpty())
                <p class="text-muted p-3 mb-0">Aucune ronde sur la période.</p>
            @else
                <table class="table table-sm mb-0">
                    <thead><tr><th>Agent</th><th>Rondes</th><th>Terminées</th><th>Anomalies</th></tr></thead>
                    <tbody>
                        @foreach ($parAgent as $ligne)
                            <tr>
                                <td>{{ $ligne['agent'] }}</td>
                                <td>{{ $ligne['total'] }}</td>
                                <td>{{ $ligne['terminees'] }}</td>
                                <td>{{ $ligne['anomalies'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
