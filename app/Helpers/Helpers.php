  ...
  public static function appClasses()
  {
    ...
    if ($layoutName !== 'blank') {
      if (isset($_COOKIE[$modeCookieName])) {
        $cookieStyle = $_COOKIE[$modeCookieName];

        // Validate the cookie value - only accept valid styles
        $validStyles = ['light', 'dark', 'system'];
        if (!in_array($cookieStyle, $validStyles)) {
          // Invalid style in cookie, fallback to default
          $styleVal = $data['myStyle'] == "dark" ? "dark" : "light";
          $styleUpdatedVal = $data['myStyle'] == "dark" ? "dark" : $data['myStyle'];
        } else {
          $styleVal = $cookieStyle;
          if ($styleVal === 'system') {
            $styleVal = isset($_COOKIE[$colorPrefCookieName]) ? $_COOKIE[$colorPrefCookieName] : 'light';
          }
          $styleUpdatedVal = $cookieStyle;
        }
      }
    }
